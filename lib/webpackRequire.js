...
function compile(config, name, sharedModulesMap, globals, cb) {
  ...
  invariant(
    typeof config.entry !== 'string' || !pathIsAbsolute(config.entry),
    'entry must be a string and an absolute path'
  );
  ...
  config.entry = config.entry || require.resolve('./webpackRequireEntrypoint.js');
